...
(function($){
	$.fn.Slide=function(options){
		...
		if(opts.effect=="scroolY"||opts.effect=="scroolTxt"){
			slideWH=slideH;
		} else if (opts.effect == "scroolX" || opts.effect == "scroolLoop" || opts.effect == "scroolTxtX") {
		...
		return this.each(function() {
			...
			//自动播放
			if (opts.autoPlay) {
				if ($(targetLi).length>1) {   //大于1的时候自动滚动 小于一的时候不滚动
			        autoPlay = setInterval(doPlay, opts.timer);//自动滚动
			    }
	...
	$.fn.Slide.effect={
		...
		scroolTxtX: function(contentObj, undefined, i, slideH, opts) {
			//alert(i*opts.steps*slideH);
			contentObj.animate({ "left": -opts.steps * slideH }, opts.speed, function() {
				for (var j = 0; j < opts.steps; j++) {
					contentObj.find("li:first").appendTo(contentObj);
				}
