...
public partial class TongJi_Settle : System.Web.UI.Page
{
    const string basesql = "select baojiacpxiangmu.id as dataid,anjianinfo2.filltime as shijishijian,BaoJiaCPXiangMu.baojiaid, baojiacpxiangmu.bumen as shiyanshi,anjianinfo2.rwbianhao, xiadariqi, (select sum(feiyong) from ceshifeikf where xmid=baojiacpxiangmu.id and taskid=anjianinfo2.rwbianhao and "
            ...
            + " AnJianInFo2.name as chanpin,anjianinfo2.xinghao as xinghao,ProjectItem.state as 项目状态,AnJianInFo2.state as 案件状态,"
            ...
            + "BaoJiaCPXiangMu.id=anjianxinxi3.xiangmubianhao left join AnJianInFo2 on anjianxinxi3.bianhao=anjianinfo2.bianhao  inner join ProjectItem on AnJianInFo2.rwbianhao=ProjectItem.taskid where anjianxinxi3.bianhao!='' and anjianinfo2.id!='' and  ProjectItem.state='完成' and ProjectItem.xmid=BaoJiaCPXiangMu.id ";
    ...
    {
        string sql = basesql + " and convert(datetime,ProjectItem.finishdate) between '" + Convert.ToDateTime(txFDate.Value) + "' and '" + Convert.ToDateTime(txTDate.Value).AddHours(23) + "' order by baojiacpxiangmu.id asc";
    ...
    protected void Button_outport_Click(object sender, EventArgs e)
    {
        ...
        base.Response.AddHeader("content-disposition", "attachment; filename=Settel" + DateTime.Now.ToShortDateString() + ".xls");
