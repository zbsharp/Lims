...
public partial class SettleProject : System.Web.UI.Page
{
    const string basesql = @"select BaoJiaCPXiangMu.id as xmid,anjianinfo2.xiadariqi as 受理日期,yaoqiuwanchengriqi as 要求完成时间,  
                             ...
                             baojiacpxiangmu.bumen as 实验室,ProjectItem.state as 项目状态,
                            ...
                            (select SUM(feiyong) from CeShiFeiKf where xmid=baojiacpxiangmu.id and taskid=anjianinfo2.rwbianhao and project='规费') as waibaofeiyong
                             ...
                             left join AnJianInFo2 on anjianxinxi3.bianhao=anjianinfo2.bianhao inner join ProjectItem on AnJianInFo2.rwbianhao=ProjectItem.taskid
                             where anjianxinxi3.bianhao!='' and anjianinfo2.id!='' and ProjectItem.state='完成' and BaoJiaCPXiangMu.id=ProjectItem.xmid and AnJianInFo2.rwbianhao not like 'D%'";
    ...
    protected void BindDataWithoutPaging()
    {
        ...
        {
            sql = basesql + " and  convert(datetime,ProjectItem.finishdate) between '" + Convert.ToDateTime(txFDate.Value) + "' and '" + Convert.ToDateTime(txTDate.Value).AddHours(23) + "' order by baojiacpxiangmu.id asc";
        }
        else if (dn == "认证部")
        {
            sql = basesql + " and BaoJiaCPXiangMu.epiboly='外包' and convert(datetime,ProjectItem.finishdate) between '" + Convert.ToDateTime(txFDate.Value) + "' and '" + Convert.ToDateTime(txTDate.Value).AddHours(23) + "' order by baojiacpxiangmu.id asc";
        }
        else
        {
            sql = basesql + " and BaoJiaCPXiangMu.bumen=(select departmentname from userinfo where username='" + Session["Username"].ToString() + "') and convert(datetime,ProjectItem.finishdate) between '" + Convert.ToDateTime(txFDate.Value) + "' and '" + Convert.ToDateTime(txTDate.Value).AddHours(23) + "' order by baojiacpxiangmu.id asc";
        }
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            e.Row.Cells[15].Text = baogaoid;//报告号
            e.Row.Cells[17].Text = gognchengjingli;//任务分派人-工程经理
            e.Row.Cells[18].Text = gognchengshi;//工程师
            e.Row.Cells[19].Text = ceshiyuan;
            if (e.Row.Cells[23].Text == null || e.Row.Cells[23].Text == "" || e.Row.Cells[23].Text == "&nbsp;")
            {
                //项目总费用
                e.Row.Cells[23].Text = "0.00";
            }
            if (e.Row.Cells[25].Text == null || e.Row.Cells[25].Text == "" || e.Row.Cells[25].Text == "&nbsp;")
            {
                //规费
            ...
            try
            {
                decimal jiancefei = Convert.ToDecimal(e.Row.Cells[23].Text);
                decimal guifei = Convert.ToDecimal(e.Row.Cells[25].Text);
                e.Row.Cells[26].Text = (jiancefei + guifei).ToString("f2");
            }
            catch
            {
            }
    ...
    protected void Button_outport_Click(object sender, EventArgs e)
    {
        ...
        base.Response.AddHeader("content-disposition", "attachment; filename=Settle" + DateTime.Now.ToShortDateString() + ".xls");
